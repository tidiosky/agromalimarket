            ...
            <div class="card">
                ...
                <div class="body">
                    <div class="row clearfix">
                        ...
                        <hr>
                        <a href="{{ url('/admin/unity') }}" title="Back"><button class="btn btn-raised btn-warning ">
                                <i class="material-icons">keyboard_backspace</i> Retour</button></a>
                        <a href="{{ url('/product/unity/' . $unity->id . '/edit') }}" title="Edit unité">
                            <button  class="btn btn-info btn-circle waves-effect waves-circle waves-float">
                                <i class="material-icons">mode_edit</i>
                            </button>
                        </a>

                        <form method="POST" action="{{ route('admin.cat.destroy',$unity->id) }}" accept-charset="UTF-8" style="display:inline">
                            {{ csrf_field() }}
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                ...
                                <tr>
                                    <td>{{ $loop->iteration or $unity->id }}</td>

                                    <td>{{ $unity->name }}</td>
                                    <td>{{ $unity->user->name }}</td>

                                    <td>
                                        <a href="{{ url('/admin/category/' . $unity->id . '/edit') }}" title="Edit categorie">
                                            <button  class="btn btn-info btn-circle waves-effect waves-circle waves-float">
                                                <i class="material-icons">mode_edit</i>
                                            </button>
                                        </a>

                                        <form method="POST" action="{{ url('product/unity' . '/' . $unity->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ csrf_field() }}
                                            <button  type="submit" title="Suppression de produit" onclick="return confirm('Vous voulez vraiment supprimer ce fichier')" class="btn btn-danger btn-circle waves-effect waves-circle waves-float">
                                                <i class="material-icons">delete</i>
                                            </button>
